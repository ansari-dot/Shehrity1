...
const PhysicalServices = ({ isDigitalSecurityActive }) => {
  ...
  useEffect(() => {
    const fetchPhysicalServices = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/service/physical`);
        ...
        const processedServices = response.data.map(service => ({
          ...
          image: service.image && !service.image.startsWith('http')
            ? `${import.meta.env.VITE_API_URL}${service.image.startsWith('/') ? '' : '/'}${service.image}`
            : service.image
        }));
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 my-20">
        {physicalServices.length > 0 ? (
          ...
          <div className="col-span-3 text-center py-10">
            <p className="text-gray-600">No physical services available at the moment.</p>
          </div>
        )}
      </div>
